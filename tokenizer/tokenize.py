...
def run():
    en_input = '/data0/xp/gec/ChineseNMT/data/train_nlpcc.src'
    en_vocab_size = 6609
    en_model_name = 'src'
    en_model_type = 'bpe'
    en_character_coverage = 0.9995
    train(en_input, en_vocab_size, en_model_name, en_model_type, en_character_coverage)

    ch_input = '/data0/xp/gec/ChineseNMT/data/train_nlpcc.trg'
    ch_vocab_size = 6609
    ch_model_name = 'trg'
    ch_model_type = 'bpe'
...
def test():
    ...
    sp.Load("/data0/xp/gec/ChineseNMT/tokenizer/trg.model")
    print(sp.EncodeAsPieces(text))
    print(sp.EncodeAsIds(text))
    a = [2048, 1, 2691, 1, 2508, 2472, 1, 2920, 1, 2812, 1, 2442]
